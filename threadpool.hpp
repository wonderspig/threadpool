...
//不支持类成员函数, 支持类静态成员函数或全局函数,Operation()函数等
class threadpool
{
...
public:
    ...
    auto commit_back(F&& f, Args&&... args) ->std::future<decltype(f(args...))>
    {
	...
	template<class F, class... Args>
	auto commit_front(F&& f, Args&&... args) ->std::future<decltype(f(args...))>
	{
		if (stoped.load())    // stop == true ??
			throw std::runtime_error("commit on ThreadPool is stopped.");
